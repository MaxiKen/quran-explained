#!/usr/bin/env python3
"""Clean the spa5k/tafsir_api Ibn Kathir JSON into per-surah .txt files.

Source : https://github.com/spa5k/tafsir_api  ->  tafsir/en-tafisr-ibn-kathir/
         114 JSON files, one per surah, one record per ayah.
Output : tafsir-ibn-kathir/NNN.txt   (committed to the repository)

Verified before cleaning: 114/114 surahs carry exactly one record per ayah and
the ayah numbers run 1..N in order, matching the 6,236 verses in data/.
"""
import json, re, pathlib, hashlib

SRC = pathlib.Path("/home/user/ik_json")
OUT = pathlib.Path("/home/user/quran-explained/tafsir-ibn-kathir")
...
def main():
    ...
    for ch in range(1, 115):
        recs = json.load(open(SRC / f"{ch}.json", encoding="utf-8"))
        recs.sort(key=lambda r: int(r["ayah"]))
        parts = [f"TAFSIR IBN KATHIR (English) — Surah {ch}: {NAMES[ch]}",
                 f"Source: spa5k/tafsir_api · tafsir/en-tafisr-ibn-kathir/{ch}.json",
        ...
        for r in recs:
            ...
            parts.append("")
        body = "\n".join(parts).rstrip() + "\n"
        (OUT / f"{ch:03d}.txt").write_text(body, encoding="utf-8")
        stats.append((ch, len(recs), len(body)))
    tot_v = sum(s[1] for s in stats)
    tot_b = sum(s[2] for s in stats)
    print(f"surahs written : {len(stats)}")
    print(f"verses written : {tot_v}")
    print(f"total size     : {tot_b/1024/1024:.2f} MB")
    print(f"sha256 (all)   : {hashlib.sha256(''.join(open(OUT/f'{c:03d}.txt',encoding='utf-8').read() for c,_,_ in stats).encode()).hexdigest()[:16]}")
    return stats
