#!/usr/bin/env python3
"""Clean the spa5k/tafsir_api English tafsir JSON into per-surah .txt files.

Source : https://github.com/spa5k/tafsir_api  ->  tafsir/en-*/N.json
         11 en-* tafsirs; per-surah JSON; records carry keys text / ayah / surah.
...
Folders written by this script (Ibn Kathir already lives in tafsir-ibn-kathir,
built by build_ik_txt.py; run this script with --check-ik to prove the two
pipelines agree byte-for-byte):

    tafsir-al-jalalayn          en-al-jalalayn                  6,236 records (full)
    tafsir-al-qushairi          en-al-qushairi-tafsir           1,290 records
    tafsir-asbab-al-nuzul       en-asbab-al-nuzul-by-al-wahidi  1,089 records (surahs 1-77 upstream)
    tafsir-kashani              en-kashani-tafsir               1,668 records
    tafsir-kashf-al-asrar       en-kashf-al-asrar-tafsir          713 records (surahs 1-69 upstream)
    tafsir-al-mukhtasar         en-tafsir-al-mukhtasar          6,236 records (full)
    tafsir-al-tustari           en-tafsir-al-tustari              888 records
    tafsir-ibn-abbas            en-tafsir-ibn-abbas             6,236 records (full)
    tafsir-maarif-ul-quran      en-tafsir-maarif-ul-quran       6,236 records (full)
    tafsir-tazkirul-quran       en-tazkirul-quran               6,236 records (full)

Verified before cleaning:
  - every record carries exactly the keys text / ayah / surah; no empty text;
  - ayah numbers run 1..N once sorted numerically (upstream stores them
    string-sorted: 1, 10, 100, ...); one record per ayah for the full tafsirs,
    6,236 records = the 6,236 verses in data/;
  - the partial tafsirs cover only part of each surah (upstream marks the rest
    under an "empty_ayahs" key, and that mark is itself incomplete in places);
    the txt keeps exactly the records the source has;
  - asbab-al-nuzul has no upstream file for surahs 78-114 and kashf-al-asrar
    none for 70-114; those surahs are written as a header plus an explicit
    "[No entry ...]" note so every folder still holds 114 files.
...
def report(name, stats):
    ...
    print(f"{name:24s} surahs {len(stats):3d}  verse-sections {tot_v:5d}  size {tot_b/1024/1024:6.2f} MB")
...
def check_ik():
    ...
    bad = []
    for ch in range(1, 115):
        a = (tmp / f"{ch:03d}.txt").read_bytes()
        b = (ref / f"{ch:03d}.txt").read_bytes()
        if a != b:
            bad.append(ch)
...
def main():
    ...
    total = 0
    for name, (title, upstream) in TAFSIRS.items():
